...
class obj_and_api
{
    ...
    public function buseness()
    {
        ...
        $objReflection->getName();
        '获取检查的类名';
        $objReflection->isUserDefined();
        '获取类是否已经定义';
        $objReflection->isInternal();
        '检测类是否是内置类';
        $objReflection->isAbstract();
        '检测类是否是抽象的';
        $objReflection->isInterface();
        '检测类是否为接口';
        $objReflection->isInstantiable();
        '检测类是否可实例化';
        $objReflection->getFileName();
        '得到类文件的绝对路径';
        $lines = file($objReflection->getFileName());//file()函数获取所有行组成的数组
        $start = $objReflection->getStartLine();
        '类文件开始的行号';
        $end = $objReflection->getEndLine();
        '类文件结束的行号';
        implode(array_slice($lines, $start - 1;
        $end));
